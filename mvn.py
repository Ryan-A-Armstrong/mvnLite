...
parser.add_argument('-s', '--silent', type=int, required=False,
                    help='1 to suppress all prints (useful in batch mode)')
...
if args['input_file'] is not None:
    IO(args['input_file'], echo_inputs=args['echo_inputs'], silent_mode=args['silent'])
...
if args['input_dir'] is not None:
    ...
    for filename in glob.glob(os.path.join(args['input_dir'], '*.txt')):
        ...
        IO(filename, echo_inputs=args['echo_inputs'], silent_mode=args['silent'])
        c_file += 1
